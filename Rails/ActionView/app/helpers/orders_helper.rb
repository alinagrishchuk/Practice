module OrdersHelper
  def orders_chart_data(start = 3.weeks.ago)
    ...
    (3.weeks.ago.to_date..Date.today).map do |date|
      {
        purchased_at: date,
        price: orders_by_date[date].first.try(:total_price) || 0,
        shipping_price: shipping_by_date[date].first.try(:total_price) || 0,
        download_price: download_by_date[date].first.try(:total_price) || 0
      }
    end
  end
end
